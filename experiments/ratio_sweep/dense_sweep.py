"""
Dense low-ratio sweep for RQ2.

Same rigorous harness as static_sweep.py (bug fix, per-example logging, exact
match, paired dF1, p50/p95, randomized order, 3 reps, separated collection cost)
but focused on the LOW-ratio regime where a quality-latency tradeoff is most
likely to appear -- including r = 0.0 (full reuse, RoPE-corrected only, suffix
tokens still recomputed since they were never cached).

The r=0.0 point is the most important: if quality holds even here, the ratio
does not measurably matter on this workload (a threat to the thesis); if quality
finally drops here, we have found the floor of the tradeoff.
"""
...
# ---------------- CONFIG ----------------
RATIOS = [0.0, 0.01, 0.02, 0.03, 0.05, 0.08, 0.10]
N_REPS = 3
OUTPUT_JSON = "dense_sweep_results.json"
SEED = 0
# ----------------------------------------
...
for ex_idx, ex in enumerate(eval_dataset):
    ...
    # ---------- PHASE 1: collect KVs ONCE, timed separately ----------
    ...
    print(f"[{ex_idx+1}/{len(eval_dataset)}] collect={collection_cost[ex_idx]*1000:.1f}ms")
...
with open(OUTPUT_JSON, "w") as f:
    json.dump({"summary": summary, "records": records,
               "full_baseline": {str(k): v for k, v in full_baseline.items()},
               "collection_cost": {str(k): v for k, v in collection_cost.items()}}, f, indent=2)
